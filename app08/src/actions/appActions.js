...
import {
    MOTIFICA_ADICIONA_CONTATO_EMAIL,
...
export const modificaAdicionaContatoEmail = (texto) => {
    return {
        type: MOTIFICA_ADICIONA_CONTATO_EMAIL,
        payload: texto
    }
}
            ...
            .then(snapshot => {
                if(snapshot.val()){
                    ...
                    firebase.database().ref(`/usuario_contatos/${emailUsuarioB64}`)
                        ...
                        .catch(e => adicionaContatoErro(e.message, dispatch))
                }
                else {
                    dispatch({ 
...
export const contatosUsuarioFetch = () => {
    const { currentUser } = firebase.auth();
    return (dispatch) => {
        let emailUsuarioB64 = b64.encode(currentUser && currentUser.email);
        firebase.database().ref(`/usuario_contatos/${emailUsuarioB64}`)
            .on('value', snapshot => {
...
export const enviaMensagem = (mensagem, contatoNome, contatoEmail) => {
    ...
    return dispatch => {
        ...
        //tipo 'e' é de envio e 'r' é de recebimento
        firebase.database().ref(`/mensagens/${usuarioEmailB64}/${contatoEmailB64}`)
            ...
            .then(() => { //armazenar cabeçalhos de conversa do usuário autenticado
                firebase.database().ref(`/usuario_conversas/${usuarioEmailB64}/${contatoEmailB64}`)
                    .set({ nome: contatoNome, email: contatoEmail })
            })
            .then(() => { //armazenar cabeçalhos de conversa do contato
